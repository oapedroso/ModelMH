...
while (stop!= True):
    y = input("Digite um elemento da liga: ")
    elements.append(y) #adiciona a entrada y no fim da lista de elementos
    x = float(input("Digite a composição do elemento {} na liga: " .format(y)))
    ...
    z = input("Deseja adicionar mais um elemento? Y/n ")
    if z == "n":
        stop = True
...
for element in alloy_composition.keys():
    ...
    alloy_name.append(str(truncate(alloy_composition[element],2)))

separator = ''
alloy = separator.join(alloy_name)
print(f"A liga inserida é: {alloy}")
...
stop = False
while (stop!= True):
    y = float(input("Digite uma temperatura em graus celsius para os cálculos: "))
    y= y + 273.15
    temperature.append(y) #adiciona a entrada y no fim da lista de elementos
    z = input("Deseja adicionar mais uma temperatura? Y/n ")
    if z == "n":
        stop = True
...
p_equilibrium_bd = []
p_notequilibrium = []
ch = input("Digite o passo na composição de hidrogênio da liga (Default:0.0005) \ninput:")
if not ch or ch == None:
    cH_step = 0.0005
else:
    cH_step = float(ch)
    
print("Descreva os parametros r e theta para cada estrutura no formato: \ntheta_alpha,r_alpha,theta_beta,r_beta,theta_delta,r_delta")
rt = (input("O padrão é 6,7,4,2,2,1 \ninput:"))
...
for T in temperature:
    ...
    for c_H in C_H:
        ...
        #E = -0.158558
        #G = 172.707974
        #S_0 = A*ln(t) + B*t + C*(t**2)/2 + D*(t**3)/3 - E/(2*t**2) + G
        #S_0 = S_0/1000 #kJ/mol
        S_0 = R * 7/2 * (1 + ln(T/9.2)) # Calculation of Hydrogen standard entropy 
        ...
        if equilibrium_real == 'ad': 
            ...
            for c_H in C_H:
                if c_H < equilibrium_all[equilibrium_real][0][0]: # intervalo fase alpha
                    ...
                    peq = P_0*exp(lnpH)
                    p_1.append([c_H,peq])
                if  equilibrium_all[equilibrium_real][0][0]<c_H<equilibrium_all[equilibrium_real][0][1]: # plato alpha - delta
                    p_2.append([c_H,peq])
            ...
            if not equilibrium_all['bd'] or saveab[0][1] > savebd[0][0]:
                for c_H in C_H:
                    if c_H < equilibrium_all[equilibrium_real][0][0]: #intervalo fase alpha
                        ...
                        peq = P_0*exp(lnpH)
                        p_1.append([c_H,peq])
                    if  equilibrium_all[equilibrium_real][0][0]<c_H<equilibrium_all[equilibrium_real][0][1]: #plato alpha-beta
                        p_2.append([c_H,peq])
                    if lim_ab < c_H < t_beta/r_beta: #intervalo fase beta
                        ...
                        if H_delta <0:
                            p_4.append([c_H,peq])
                            p_5.append([c_H,peq])
                    if t_beta/r_beta < c_H < C_H[-1]: # plato beta-delta
                        p_4.append([c_H,peq])
                        p_5 =  [[c_H,peq]]    
                lastcH_delta = C_H[-2]
                if H_delta > 0:
                ...
                for c_H in C_H:
                    ...
                    if lim_ab < c_H < equilibrium_bd[0]: #intervalo fase beta
                        ...
                        peq = P_0*exp(lnpH)
                        p_3.append([c_H,peq])
                    if equilibrium_bd[0] < c_H < equilibrium_bd[1]: # plato beta-delta
                        p_4.append([c_H,peq])
                    if equilibrium_bd[1] <c_H< lim_delta: # intervalo fase delta
                        mu_delta = float(val_mu_H_delta[T][np.where(val_mu_H_delta[T]==c_H)[0],1])
                        lnpH = 2* mu_delta/(R*T)
                        peq = P_0*exp(lnpH)
                        p_5.append([c_H,peq])
