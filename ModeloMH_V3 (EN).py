...
while (stop!= True):
    y = input("Enter an element of the alloy: ")
    elements.append(y) #adiciona a entrada y no fim da lista de elementos
    x = float(input("Enter element {} composition in the alloy: " .format(y)))
    ...
    z = input("Do you want to add one more element? Y/n ")
    if z == "n":
        stop = True
...
while (stop!= True):
    ...
    z = input("Do you want to add one more temperature ? Y/n ")
    if z == "n":
        stop = True
...
p_equilibrium_bd = []
p_notequilibrium = []
ch = input("Enter the step in the hydrogen composition of the alloy  (Default:0.0005) \ninput:")
if not ch or ch == None:
    cH_step = 0.0005
else:
    cH_step = float(ch)
    
print("Enter the r and theta parameters for each structure in the format: \ntheta_alpha,r_alpha,theta_beta,r_beta,theta_delta,r_delta")
